...
print('【测试HTTP访问】')
exit_code, out, err = run_cmd(client, 'curl -s http://localhost:9443/ 2>&1 | head -20')
print(out.strip() if out else '无')

print('\n【测试sing-box JSON】')
exit_code, out, err = run_cmd(client, 'curl -s http://localhost:9443/singbox/JP 2>&1 | head -50')
print(out.strip() if out else '无')
